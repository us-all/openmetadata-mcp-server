...
- **v1.16.10** (2026-06-19): MCP tool annotations 적용 — `@us-all/mcp-toolkit ^1.3.0`의 `inferToolAnnotations`를 중앙 `tool()` 헬퍼에 추가, 전 도구(173)에 readOnlyHint/destructiveHint/openWorldHint 자동 부여 (override 0). annotations가 tools/list 응답에 도구당 hint 필드를 추가해 default-all 토큰이 ~28.9k→30.9k(+~6%)로 증가(typical 시나리오는 23k 유지) → CI `TOKEN_BUDGET` 29000→32000 상향(v1.16.1 이후 두 번째 조정). 비파괴 패치. 60/60 test.
